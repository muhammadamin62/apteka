...
bot = Bot(token=TOKEN)
dp = Dispatcher()
# Используем таймзону Ташкента для всего бота
...
async def send_reminder(chat_id: int, med_id: int, med_name: str, time_val: str):
    ...
    # Удаляем старые повторы перед отправкой нового
    ...
    msg = (f"✨ **VAQT KELDI** ✨\n\n"
           f"💊 Dori nomi: **{med_name}**\n"
           f"🕒 Rejadagi vaqt: {time_val}\n\n"
           f"*Dori ichishni unutmang!*")
    
    try:
        await bot.send_message(chat_id, msg, reply_markup=builder.as_markup(), parse_mode="Markdown")
        
        # Повтор через 2 минуты (назойливый режим), пока не нажмет кнопку
        run_at = datetime.now(scheduler.timezone) + timedelta(minutes=2)
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    for i in range(1, 5): 
        builder.button(text=f"{i} mahal", callback_data=f"f_{i}")
    builder.adjust(2)
    await message.answer(f"🔢 **{message.text}**\nKuniga necha mahal ichasiz?", reply_markup=builder.as_markup(), parse_mode="Markdown")
...
async def process_freq(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"🕒 **1-qabul vaqtini** kiriting (masalan, 08:00):", parse_mode="Markdown")
...
async def process_times(message: types.Message, state: FSMContext):
    ...
    if ":" not in message.text:
        return await message.answer("❌ Iltimos, vaqtni HH:MM formatida yozing (masalan, 14:30)")
    
    times.append(message.text.strip())
    
    if len(times) < data['freq']:
        await state.update_data(times=times)
        await message.answer(f"🕒 **{len(times)+1}-qabul vaqtini** kiriting:", parse_mode="Markdown")
    else:
        await state.update_data(times=times)
        await message.answer("📦 Qutida nechta dori (tabletka) bor?", parse_mode="Markdown")
...
async def process_stock(message: types.Message, state: FSMContext):
    ...
    with sqlite3.connect("med_bot.db") as conn:
        for t in data['times']:
            ...
            try:
                h, m = map(int, t.split(":"))
                # Планируем ежедневное напоминание (cron)
                scheduler.add_job(send_reminder, "cron", hour=h, minute=m, 
                                  args=[message.chat.id, med_id, data['name'], t], id=f"main_{med_id}")
            except Exception as e:
...
async def med_done(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("🌈 **Sog' bo'ling!** Dori ichilgani qayd etildi.", parse_mode="Markdown")
    await callback.answer()

@dp.callback_query(F.data.startswith("snooze_"))
async def med_snooze(callback: types.CallbackQuery):
    data = callback.data.split("_")
    med_id, name, t_val = data[1], data[2], data[3]
    
    # Отключаем текущий повтор
    nag_id = f"nag_{med_id}"
    if scheduler.get_job(nag_id): 
        scheduler.remove_job(nag_id)
    
    # Ставим новое разовое напоминание через 15 минут
    new_time = datetime.now(scheduler.timezone) + timedelta(minutes=15)
    scheduler.add_job(send_reminder, "date", run_date=new_time, 
                      args=[callback.message.chat.id, int(med_id), name, t_val], id=f"snz_{med_id}")
    
    await callback.message.edit_text(f"⏳ Eslatma 15 daqiqaga surildi. Yangi vaqt: {new_time.strftime('%H:%M')}")
    await callback.answer()
...
async def med_del(callback: types.CallbackQuery):
    ...
    if scheduler.get_job(f"main_{mid}"): 
        scheduler.remove_job(f"main_{mid}")

...
bot = Bot(token=TOKEN)
dp = Dispatcher()
# Railway серверы обычно работают по UTC, поэтому Asia/Tashkent важен!
...
async def cmd_start(message: types.Message):
    await message.answer(
        "👋 Привет! Я твой ИИ-ассистент по лекарствам.\n\n"
        "📸 Пришли фото инструкции\n"
...
async def handle_photo(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        prompt = "Это фото лекарства. Напиши только название и время приема (ЧЧ:ММ). Пример: 'Анальгин 08:00'. Если данных нет, предложи 09:00."
        response = ai_model.generate_content([prompt, {"mime_type": "image/jpeg", "data": photo_bytes.getvalue()}])

        await msg.edit_text(f"🤖 **Совет ИИ:**\n{response.text}\n\nВведите название лекарства (или скопируйте у ИИ):")
        await state.set_state(MedStates.waiting_name)
    except Exception as e:
        logger.error(f"Gemini error: {e}")
        await msg.edit_text("❌ Ошибка анализа. Введите данные вручную через /add")


@dp.message(MedStates.waiting_name)
async def add_name(message: types.Message, state: FSMContext):
    await state.update_data(name=message.text)
    await message.answer("Введите время приема (ЧЧ:ММ), например 08:00:")
...
async def add_time(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("Количество таблеток в упаковке (число):")
        await state.set_state(MedStates.waiting_stock)
    except:
        await message.answer("❌ Неверный формат! Нужно ЧЧ:ММ (например 20:30)")
...
async def add_stock(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ Напоминание установлено: **{data['name']}** в {data['time']}")
    await state.clear()
...
async def handle_callback(callback: types.CallbackQuery):
    ...
    with get_db() as conn:
        ...
        if not res:
            return await callback.answer("Ошибка: не найдено.")
        ...
        if action == "done":
            ...
            msg = f"✅ Принято! Остаток: {new_stock} шт."
            if new_stock <= 3: msg += "\n⚠️ Заканчивается!"
            await callback.message.edit_text(msg)

        elif action == "snooze":
            run_date = datetime.now() + timedelta(minutes=15)
            scheduler.add_job(send_reminder, "date", run_date=run_date, args=[callback.message.chat.id, med_id, res[0]])
